...
import { PaymentStreakTracker } from '@/components/PaymentStreakTracker';
...
export default function LandlordDashboard() {
  const { user } = useAuth();
  const { properties, units, isLoading } = useProperties();
  const [refreshing, setRefreshing] = useState(false);
  const [activeTab, setActiveTab] = useState<'overview' | 'vendors' | 'compliance' | 'predictive'>('overview');
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'vendors':
        return (
          <VendorMarketplace
            onVendorSelect={(vendor) => console.log('Selected vendor:', vendor)}
            onBookNow={(vendor) => console.log('Book vendor:', vendor)}
          />
        );
      case 'compliance':
        return (
          <ComplianceCenter
            ...
            onItemPress={(item) => console.log('Compliance item:', item)}
            onScheduleReminder={(item) => console.log('Schedule reminder:', item)}
          />
        );
      case 'predictive':
        return (
          <PredictiveMaintenance
            propertyId="prop-1"
            onScheduleMaintenance={(alert) => console.log('Schedule maintenance:', alert)}
        ...
        return (
          ...
          >
            ...
            <PaymentStreakTracker
              ...
              onShareStreak={() => console.log('Share streak')}
            />
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Landlord Dashboard', headerShown: true }} />
        ...
        >
          ...
          <View style={styles.tabsContainer}>
            ...
            >
              <TabButton
                tab="overview"
                icon={<Home size={20} color={activeTab === 'overview' ? theme.colors.primary : theme.colors.text.secondary} />}
                title="Overview"
                isActive={activeTab === 'overview'}
              />
              <TabButton
                tab="vendors"
                icon={<Wrench size={20} color={activeTab === 'vendors' ? theme.colors.primary : theme.colors.text.secondary} />}
                title="Vendors"
                isActive={activeTab === 'vendors'}
              />
              <TabButton
                tab="compliance"
                icon={<Shield size={20} color={activeTab === 'compliance' ? theme.colors.primary : theme.colors.text.secondary} />}
                title="Compliance"
                isActive={activeTab === 'compliance'}
              />
              <TabButton
                tab="predictive"
                icon={<Brain size={20} color={activeTab === 'predictive' ? theme.colors.primary : theme.colors.text.secondary} />}
                title="AI Insights"
...
const styles = StyleSheet.create({
  ...
  tabIcon: {
    // Icon container for tab buttons
  },
});